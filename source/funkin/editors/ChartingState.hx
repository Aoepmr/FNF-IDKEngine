...
import funkin.editors.content.SimplexNotePreview;
import funkin.editors.content.FunkinSlider;
...
/*
	TO DO:
	- Update note preview when undoing/redoing actions and editing selected notes.
	- Find a better way to handle the welcome music.
	- Improve the playbar.
	- Color quantization in the notes?
	- Add zoom levels to the note preview for better visibility in larger charts.
	- Optimize.
*/
...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	var notePreview:SimplexNotePreview;
	...
	var shouldPlayWelcomeMusic:Bool = true;
	...
	override function create()
	{
		...
		welcomeMusic.loadEmbedded(Paths.music('chartEditorLoop'));
		...
		notePreview = new SimplexNotePreview(0, 0, FlxG.height);
		notePreview.scrollFactor.set();
		notePreview.screenCenter();
		notePreview.x += 240;
		add(notePreview);

		notePreview.clear();

		playBarHead = new FunkinSlider(0, 0, FlxG.height, 0, 1, Std.int(notePreview.width));
		playBarHead.scrollFactor.set();
		playBarHead.setThumbSize(Std.int(notePreview.width * 1.7), 24);
		playBarHead.screenCenter();
		playBarHead.x += 254;
		playBarHead.lerpSpeed = 4;
		playBarHead.showTrack = false;
		playBarHead.onChange = function(v:Float)
		{
			...
			var newPos:Float = v * songLength;

			Conductor.songPosition = newPos;
			FlxG.sound.music.time = newPos + 0.000001;

			var newSec:Int = 0;
			for (i in 0...cachedSectionTimes.length)
			{
				if (cachedSectionTimes[i] <= newPos)
					newSec = i;
				else
					break;
			}
		...
		if (FlxG.sound.music != null)
		{
			if (PsychUIInputText.focusOn == null) // If not typing anything
			{
				if (FlxG.sound.music.playing)
					playBarHead.value = Conductor.songPosition / FlxG.sound.music.length;
				...
				|| (isMovingNotes && (FlxG.mouse.justPressedRight || FlxG.keys.justPressed.ESCAPE))) // Delete button
			{
				if (selectedNotes.length > 0)
				{
					...
					while (selectedNotes.length > 0)
					{
						...
						if (!note.isEvent)
						{
							notePreview.eraseNote(note.noteData, note.mustPress, note.strumTime, FlxG.sound.music.length);
							notes.remove(note);
				...
				else if (FlxG.mouse.x >= gridBg.x && FlxG.mouse.x < gridBg.x + gridBg.width)
				{
					...
					if (closest != null && (!closest.isEvent || !lockedEvents))
					{
						...
						else if (!FlxG.keys.pressed.CONTROL) // Remove Note/Event
						{
							...
							FlxG.sound.play(Paths.sound('editorSounds/noteErase'), 0.7);
							if (!closest.isEvent)
							{
								notes.remove(closest);
								notePreview.eraseNote(closest.noteData, closest.mustPress, closest.strumTime, FlxG.sound.music.length);
							}
							else
							{
					...
					else if (!holdingAlt && FlxG.mouse.y >= gridBg.y && FlxG.mouse.y < gridBg.y + gridBg.height) // Add note
					{
						...
						if (noteData >= 0)
						{
							...
							FlxG.sound.play(Paths.sound('editorSounds/noteLay'), 0.7);
							var didAdd:Bool = false;
	...
	function createNote(note:Dynamic, ?secNum:Null<Int> = null)
	{
		...
		notePreview.addNote(swagNote.noteData, swagNote.mustPress, swagNote.strumTime, FlxG.sound.music.length);
		return swagNote;
	}
	...
	override function closeSubState()
	{
		...
		// The return of Welcome Music
		shouldPlayWelcomeMusic = true;
		fadeInWelcomeMusic();
	...
	function undo()
	{
		...
		FlxG.sound.play(Paths.sound('editorSounds/undo'), 0.7);
		currentUndo++;
	}
